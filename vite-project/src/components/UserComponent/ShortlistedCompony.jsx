...
function ShortlistedCompony() {
  ...
  async function fetchData() {
    try {
      const response = await axios.get(
        "http://localhost:3000/api/ShortlistedCompony",
        {
  ...
  const shortlistedJobs = data?.filter((job) => job.Selected.includes(userId));

  return (
    <div className="min-h-screen px-4 sm:px-6 lg:px-8">
      <h1 className="text-2xl font-semibold font-mono text-center mt-8 mb-3">
      ...
      <div className="overflow-x-auto">
        <Table className="min-w-full shadow-lg rounded-lg border border-gray-300">
          <TableHeader className="bg-gray-300">
            <TableRow>
              <TableHead className="w-12 text-center text-lg font-semibold">
                S.No
              </TableHead>
              <TableHead className="w-1/6 text-lg font-semibold">
                Company Logo
              </TableHead>
              <TableHead className="w-1/6 text-lg font-semibold">
                Company Name
          ...
          <TableBody>
            {shortlistedJobs.map((job, idx) => (
              ...
              >
                ...
                <TableCell className="text-gray-700 font-semibold whitespace-nowrap">
                  {job.Salary}
                </TableCell>
